...
listFromStack :: Stack -> [Char]
listFromStack (Stack xs) = xs


stackStateMove :: Int -> Int -> Int -> StackState -> StackState
stackStateMove n from to (StackState stacks) = StackState $ replaceIndex to newTo (replaceIndex from newFrom stacks)
    where
        newFrom = Stack $ drop n (listFromStack $ stacks!!from)
        newTo = Stack $ (take n (listFromStack $ stacks!!from)) ++ (listFromStack $ stacks!!to)
