...
import Data.Array (catMaybes, concat, filter, length, mapWithIndex, zip, (!!))
...
codePointToCell cp = if cp == codePointFromChar '@' then Roll else Empty

createCells :: String -> Array (Array Cell)
createCells xs =
  let
    rows = (filter (_ /= "") $ split (Pattern "\n") xs)
  in
    map (\row -> map codePointToCell (toCodePointArray row)) rows
