...
solution :: Integer -> Tree -> Integer
solution threshold (File _ s) = 0
solution threshold d@(Directory _ s)
    | totalSize d <= threshold = (sum $ map (solution threshold) s) + totalSize d
    | otherwise                = (sum $ map (solution threshold) s)
